<%@page import="eventi.Evento, java.util.ArrayList, java.util.HashMap"%>
<%
...
<div style="border: 1px solid #ccc;" id="containerItemElencoEventi" class="col-xs-12 col-sm-4">
    ...
    <div class="card-block">
        ...
        <small class="text-muted">Evento del <b><%=new java.text.SimpleDateFormat("dd-MM-yyyy")
							.format(eventiDaMostrare.get(i).getDataEvento())%></b> alle ore <b><%=new java.text.SimpleDateFormat("HH:mm:ss").format(eventiDaMostrare.get(i).getDataEvento())
							.substring(0, 8)%></b> a <b><%=eventiDaMostrare.get(i).getComune()%>, <%=eventiDaMostrare.get(i).getProvincia()%>, <%=eventiDaMostrare.get(i).getRegione()%></b></small> <small class="text-muted row">In via <b><%=eventiDaMostrare.get(i).getIndirizzo()%></b></small>
        <%
    ...
    <div class="card-footer">
        <small class="text-muted">Pubblicato <%=new java.text.SimpleDateFormat("dd-MM-yyyy")
							.format(eventiDaMostrare.get(i).getDataPubblicazioneEvento())%> alle <%=new java.text.SimpleDateFormat("HH:mm:ss")
							.format(eventiDaMostrare.get(i).getDataPubblicazioneEvento()).substring(0, 8)%></small>
    </div>
</div>
<%
	}
%>
<div class="col-xs-12 pull-right" id="divMostraNumeroEventiPresenti" style="text-align: right;">
    <h4>
        Eventi totali presenti: <b id="numeroTotaleEventiPresenti"><%=numeroEventiTotali%></b>
    </h4>
</div>
<script>
