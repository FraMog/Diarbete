...
<img class="card-img-top center-block" src="img/eventi/<%=ultimoEventoInserito.getSrcEvento()%>" alt="Card image cap">
    <div class="card-block">
      ...
      <small class="text-muted">Evento del <b><%=new java.text.SimpleDateFormat("dd-MM-yyyy").format(ultimoEventoInserito.getDataEvento())%></b> alle ore <b><%= new java.text.SimpleDateFormat("HH:mm:ss").format(ultimoEventoInserito.getDataEvento()).substring(0,8)%></b> a <b><%=ultimoEventoInserito.getComune()%>, <%=ultimoEventoInserito.getProvincia()%>, <%=ultimoEventoInserito.getRegione()%></b></small>
      <small class="text-muted row">In via <b><%=ultimoEventoInserito.getIndirizzo()%></b></small>
